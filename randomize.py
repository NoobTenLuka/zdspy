...
def randomize(seed, workdir, outdir, enableBanlist=True, randoType="nll"):
    ...
    banlist = ["battle00", "battle01", "battle02", "battle03", "battle04", "battle05", "battle06", "battle07",
               "battle08", "battle09", "battle10", "battle11", "player_dngn", "demo_op", "demo_chase", "demo_end",
               "demo_title"]
...
def nologic(warp_list):
    ...
    for i in range(len(warp_list)):
        ...
        n_warp_ce = zds.ZMB_WARP_CE(warp_ce.UID, warp_ce.fade_type, p_warp_ce.map_id, p_warp_ce.destination_warp_id,
                                    p_warp_ce.destination, warp_ce.run_direction)
...
def nologicdual(warp_list):  # Similar to nl
    ...
    for i in range(int(round(len(warp_list) / 2))):
        ...
        # uid, ft, mid, dwid, dest, rundir
        n_warpl[fname] = zds.ZMB_WARP_CE(warp_ce.UID, warp_ce.fade_type, p_warp_ce.map_id,
                                         p_warp_ce.destination_warp_id, p_warp_ce.destination, warp_ce.run_direction)
        n_warpl[p_fname] = zds.ZMB_WARP_CE(p_warp_ce.UID, p_warp_ce.fade_type, warp_ce.map_id,
                                           warp_ce.destination_warp_id, warp_ce.destination, p_warp_ce.run_direction)
    if len(n_warpl) < len(o_warpl):
        fname, warp_ce = random.choice(list(warp_list.items()))
        n_warpl[fname] = zds.ZMB_WARP_CE(warp_ce.UID, warp_ce.fade_type, warp_ce.map_id, warp_ce.destination_warp_id,
                                         warp_ce.destination, warp_ce.run_direction)
...
def nologiclinked(warp_list):
    ...
    for i in range(int(round(len(warp_list) / 2))):
        ...
        # uid, ft, mid, dwid, dest, rundir
        new_warp_list[fname] = zds.ZMB_WARP_CE(warp_ce.UID, warp_ce.fade_type, int(p_info.map_id),
                                               p_warp_ce.destination_warp_id, p_info.level_name, warp_ce.run_direction)
        new_warp_list[p_fname] = zds.ZMB_WARP_CE(p_warp_ce.UID, p_warp_ce.fade_type, int(info.map_id),
                                                 warp_ce.destination_warp_id, info.level_name, p_warp_ce.run_direction)
    if len(new_warp_list) < len(original_warp_list_copy):
        fname, warp_ce = random.choice(list(warp_list.items()))
        new_warp_list[fname] = zds.ZMB_WARP_CE(warp_ce.UID, warp_ce.fade_type, warp_ce.map_id,
                                               warp_ce.destination_warp_id, warp_ce.destination, warp_ce.run_direction)
